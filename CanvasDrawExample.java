...
public class CanvasDrawExample extends MIDlet implements CommandListener{
  ...
  if(listItemIndex == 0){
  g.drawString("Draw line", getWidth () / 2, 5, Graphics.HCENTER | 
  Graphics.TOP); 
  g.drawLine(getWidth()/4, 100, 3 * getWidth()/4, 100);
  }

  if(listItemIndex == 1){
  g.drawString("Draw Rectangle", getWidth()/2, 5, Graphics.HCENTER | 
   Graphics.TOP);
  ...
  if(listItemIndex==2){
  g.drawString("Draw Rounded Rectangle", getWidth () / 2, 5, 
  ...
  if(listItemIndex == 3){
  g.drawString("Draw Arc", getWidth () / 2, 5, Graphics.HCENTER | 
   Graphics.TOP);  
  ...
  if(listItemIndex == 4){
  g.drawString("Draw Traingle", getWidth () / 2, 5, Graphics.HCENTER | 
   Graphics.TOP);
